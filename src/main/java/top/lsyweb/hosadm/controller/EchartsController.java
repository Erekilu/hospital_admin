...
public class EchartsController
{
	...
	public Map<String, Object> getPatientsByInp() {
		...
		map.putAll(patientService.queryPatientByTime(departmentId));
		// 获取当前部门下的男女病人数目
		map.putAll(patientService.querySexByInp(departmentId));
		// 获取当前部门并分配了病床的病人数量
		map.put("haveBed", patientService.queryPatientIfBed(departmentId));
		// 获取当前部门所有病房信息
		...
		map.putAll(patientService.queryPatientInHos(departmentId));
		map.put("code", 0);
		return map;
	}
}
